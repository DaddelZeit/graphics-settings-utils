...
M.exportInfo = im.BoolPtr(settings.getValue("zeit_graphics_export_info", true))
M.exportFormat = settings.getValue("zeit_graphics_export_option", 1)
M.exportFormats = {
    "Default",
    "BeamNG Forums",
    "Discord"
    ...
    if im.BeginCombo("##ExportFormatSelector", M.exportFormats[M.exportFormat]) then
        for k,v in pairs(M.exportFormats) do
            if im.Selectable1(v, k == M.exportFormat) then
                M.exportFormat = k
                settings.setValue("zeit_graphics_export_option", k)
            end
        end
        im.EndCombo()
    end
...
local function infoCheckbox()
    ...
    settings.setValue("zeit_graphics_export_info", M.exportInfo[0])
end
...
local function exportProfileToClipboard(name, format, exportInfo)
    ...
    if tryLpack then
        ...
        success, strData = pcall(lpack.encode, data)
...
local function importProfileFromClipboard()
    ...
    if tryLpack then
        ...
        success, data = pcall(lpack.decode, decompressRes)
