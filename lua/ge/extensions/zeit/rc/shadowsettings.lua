...
local function onUpdate()
    if not scenetree.sunsky or not values.logWeight then return end
    if scenetree.sunsky:getField("logWeight", 0) ~= values.logWeight and not editor.isEditorActive() and not zeit_rcMain.uiActive then
        scenetree.sunsky:setField("logWeight", 0, values.logWeight)
    end
end
