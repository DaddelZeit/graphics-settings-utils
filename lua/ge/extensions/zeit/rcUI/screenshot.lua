...
local function screenshot(isSteam)
...
local function toggleUI(...)
    widgets.blurRemove("zeitRenderSettingsPhoto")
...
local function renderTopBar()
    ...
    im.SetCursorPosX(width-im.CalcTextSize("X").x-style.FramePadding.x)
...
local function render(dt)
    ...
    widgets.blurUpdate("zeitRenderSettingsPhoto")
    im.BeginMenuBar()
    renderTopBar()
    im.EndMenuBar()
    ...
    im.SetWindowFontScale(1)
    if Steam and Steam.isWorking and Steam.accountID ~= "" and widgets.button("Steam Screenshot", im.ImVec2(im.GetContentRegionAvailWidth(), im.GetTextLineHeightWithSpacing()*1.25)) then
        screenshot(true)
    end
    im.PopStyleColor()
    im.PopFont()
...
local function onZeitGraphicsLoaded()
    if zeit_rcUI_select then
        zeit_rcUI_select.addEntry("screenshot", {
