...
local blockAutoClose = false
local function toggleUI(show, _blockClose, func)
    if _blockClose ~= nil then
        blockAutoClose = _blockClose
    end

    if not show and blockAutoClose then return end
    M.showUI = show
...
local function render(dt)
    ...
    if active == 0 then
        ...
        local temp = im.BoolPtr(not settingsManager.get("delete_dialog_show"))
        if im.Checkbox("Do not show this again", temp) then
            settingsManager.set("delete_dialog_show", not temp[0])
        end
        im.SameLine()
