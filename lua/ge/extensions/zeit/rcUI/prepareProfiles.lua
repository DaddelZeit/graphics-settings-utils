...
local function prep(tbl, onFinish, ignoreCache)
    ...
    core_jobsystem.wrap(function(job)
        for _,fullPath in ipairs(tbl) do
            local path = fullPath:gsub(zeit_rcMain.profilePath, "")
            local internalName = split(path, ".")[1]
            local jsonInfo = zeit_rcMain.loadInfo(internalName) or {}
            local info = {
            ...
            if ignoreCache or not previewCache[previewPath] then
                -- the game does already cache the image buffers, not the texObj tables however
                -- since we need it anyways, we cache the *entire* thing
                previewCache[previewPath] = imguiUtils.texObj(previewPath)
            end

            local overrides = FS:findOverrides(fullPath)
            ...
            -- 0: included
            local fileSource = 1
            if filePath:match(zeit_rcMain.modInstallPath:gsub("/", "\\")) then
                fileSource = 0
            elseif filePath == FS:getUserPath() then
                fileSource = 2
            end

            profiles[internalName] = {
                info = info,
                preview = previewCache[previewPath],
