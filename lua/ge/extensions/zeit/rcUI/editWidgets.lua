...
local function blurUpdate(id)
    ...
    ui_gameBlur.replaceGroup(id, {["1"] = data})
end
M.blurUpdate = blurUpdate

local function blurRemove(id)
    if ui_gameBlur then
        ui_gameBlur.replaceGroup(id, nil)
    end
end
M.blurRemove = blurRemove

local function guardViewportOverflow()
    local mainPort = im.GetMainViewport()

    local windowMin = im.GetWindowPos()
    local windowSize = im.GetWindowSize()
    local windowMax = im.ImVec2(windowMin.x+im.GetWindowWidth(), windowMin.y+im.GetWindowHeight())
    if windowMax.x > mainPort.Pos.x + mainPort.Size.x or windowMax.y > mainPort.Pos.y + mainPort.Size.y then
        im.SetWindowPos1(im.ImVec2(math.min(windowMax.x, mainPort.Pos.x + mainPort.Size.x)-windowSize.x, math.min(windowMax.y, mainPort.Pos.y + mainPort.Size.y)-windowSize.y), im.Cond_Always)
    end

    if windowMin.x < mainPort.Pos.x or windowMin.y < mainPort.Pos.y then
        im.SetWindowPos1(im.ImVec2(math.max(windowMin.x, mainPort.Pos.x), math.max(windowMin.y, mainPort.Pos.y)), im.Cond_Always)
    end
end
M.guardViewportOverflow = guardViewportOverflow
...
local function textCenteredLocal(text)
    local areaWidth = im.GetContentRegionAvailWidth()
    local textWidth = im.CalcTextSize(text).x

    im.SetCursorPosX(im.GetCursorPosX()+(areaWidth - textWidth)/2)
    im.Text(text)
end
M.textCenteredLocal = textCenteredLocal
