        ...
        if im.CollapsingHeader1("Letterbox") then
            ...
            widgets.renderFloatGeneric(
                "LetterboxPostFX.uvY1, LetterboxPostFX.uvY2", {
                    ...
                    if zeit_rc_letterbox then zeit_rc_letterbox.getAndSaveSettings(nil, {customPFXVals.LetterboxColorPFX[0], customPFXVals.LetterboxColorPFX[1], customPFXVals.LetterboxColorPFX[2]}) end
                end)
            ...
            do
                ...
                if widgets.resetButton("LetterboxPostFX.Color") then
                    if zeit_rc_letterbox then zeit_rc_letterbox.getAndSaveSettings(customPFXVals.HeightPFXValue[0], {0,0,0}) end
                end
                if disabled then im.EndDisabled() end
                im.SameLine()
...
local function render(dtReal)
    im.Begin("Zeit's Graphics Utils: Profile Editor", nil, im.WindowFlags_AlwaysAutoResize + im.WindowFlags_MenuBar)
    ...
    im.Separator()
    if im.TreeNodeEx1("Help", im.TreeNodeFlags_SpanFullWidth) then
